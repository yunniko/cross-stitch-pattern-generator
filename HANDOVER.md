...
Last verified: 2026-09-25 at 354c7c2 (G-073 complete, M1-M6; deployed and exercised live)
...
| 2026-09-25 | db369dc | **G-073 M5: backstitch on the exports, and the legends split.** The chart PNGs and A4 pages draw it at a fifth of a cell with five dash patterns, beads carrying the thread's symbol and a casing where it crosses cells of its own lightness (D233); the Pattern Keeper PDF keeps it off the grid and reports it as text. The simple legend is now a thread consumption table, the extended one carries per-thread backstitch lengths and the chart's total (D234). Includes two fixes: export-time palette compaction dropped backstitch-only threads, and drawing inside `draw_chart` leaked onto the PK PDF | 824 unit (10 new), 432 e2e, 73 golden hashes unmoved, tsc, eslint, prettier, docs-lint, cargo clippy and cargo fmt on the touched crate. **Criteria 6 and 7 conflict and 6 wins**: measured against the previous binary, a chart with no backstitch exports byte-identical PNGs and OXS, while A4 and PDF differ only on their legend pages; the PK PDF is byte-identical whether or not the chart has backstitch. The two copies of the dash table are compared through the real binary by `test:backstitch-style:rust`, now in CI and mutation-checked. Live on the deployed build (both containers rebuilt): drew three backstitch threads, saw them in the thread list with their lengths, exported the full chart PNG through the live processor and read a yellow dotted line with its symbol beads off the returned image. 23 containers before and after with an identical name set; ten sites returned 200. |
