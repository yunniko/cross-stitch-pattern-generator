...
Last verified: 2026-09-19 at a2794c5 (G-047 M2: the realistic preview streamed from tiles, deployed and verified live)
...
**Production** runs a2794c5 (2026-09-19), the last deployed commit: the 1b shell with the Owner's corrections, and generation, the enhancement preview and every export but the editable save running in the `processor` container.
...
**Checks run 2026-09-19**: `tsc --noEmit` clean, `npm run lint` 0 errors, production build clean; Vitest 1075
...
the A4 export takes 62.1 s (126.0 s before G-047 M1, D171, D172) and Export all 201.1 s.
...
- G-047 (faster exports and generation, from `docs/reviews/2026-09-19-algorithm-review.md`) is ACTIVE: M1 (PNG writer, symbol stamps; live A4 at 1000 in 62.1 s, accepted by the Owner at zlib level 6) and M2 (the streamed preview) deployed 2026-09-19. M3, the Pattern Keeper PDF without pdf-lib's per-operator bookkeeping, is next.
