...
Decision: `buildPattern` builds `lib/pipeline-context.ts`'s context once (true cells, interleaved Float64 `cellOklab`, importance, pair evidence, crisp layer); ICM, contour cleanup, denoise, crisp finalization and brand re-optimization take it as their first argument, with per-stage options after. Denoise returns its own OKLab for the quantizer, which accepts an optional precomputed buffer.
...
Evidence: lib/pipeline-context.ts; tests/unit/golden-hashes.spec.ts; tests/unit/denoise.spec.ts.
