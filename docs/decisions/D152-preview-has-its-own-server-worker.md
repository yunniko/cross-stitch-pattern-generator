...
Date: 2026-09-17 · Goal: G-034 M3 · Status: superseded by D240 (the preview it served is gone)
...
Decision: the processor runs previews on a dedicated worker (preview-runner.ts, preview-worker.ts) with its own short queue and deadline, and caches the encoded result per photo and mode (preview-cache.ts). This mirrors D116, which gave the browser preview its own worker for the same reason.
...
Evidence: docs/decisions/D149-server-processing-caps-and-pool.md; docs/decisions/D240-the-enhancement-modes-are-removed.md
