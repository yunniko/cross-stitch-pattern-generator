...
Force: judgment — an addon would work, but native code cannot be interrupted, so a cancelled job would keep burning a core, and a segfault would take the worker down; a child dies with `kill`.
