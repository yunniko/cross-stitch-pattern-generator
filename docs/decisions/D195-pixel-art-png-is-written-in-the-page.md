...
Context: every other raster export runs on the processor (D149), now in Rust (D190). This one is a single pass over the cells, with no font, layout or grid.
...
Consequence: the browser holds the whole image while encoding, about 9 MB at the 1500-stitch cap. A kind needing the processor goes in `ExportKind`, not here.
