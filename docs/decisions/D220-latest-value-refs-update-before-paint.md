...
Rejected: raising the test's timeout (hides a real dropped keystroke); writing the ref during render (React discourages it, and a layout effect closes the same window).
Consequence: anything mirroring state into a ref for an event handler uses `useLatest` or a layout effect. A passive effect is wrong for state a handler reads.
