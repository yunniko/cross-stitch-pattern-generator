...
### G-070 · Rust stops being shaped by a language that is no longer here — CLOSED, ANSWERED (2026-09-24, Owner sign-off 2026-09-24)
...
- 2026-09-24 — **Owner closed the goal as answered.** M1 was the whole of it: the question "what does
  byte-identity with V8 still cost?" has a measured answer — nothing; it is the faster implementation (D223).
  M2–M5 are not done and should not be: they were premised on a win that does not exist. Criteria 3–5 are
  therefore **not met and deliberately abandoned**, which is the honest reading rather than DONE. The one
  finding worth acting on — a `target-cpu` baseline, ~5% with byte-identical output — is **G-071**.
