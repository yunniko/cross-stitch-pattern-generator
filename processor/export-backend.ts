...
export const serverExportBackend: ExportBackend = {
  ...
  async pixelsToPngBlob(source: PixelSource, width: number, height: number): Promise<Blob> {
    // Strips go straight to the encoder; the image is never assembled (G-047 M2).
    const bytes = await encodePng(source, width, height);
    return new Blob([bytes as Uint8Array<ArrayBuffer>], { type: "image/png" });
  },
