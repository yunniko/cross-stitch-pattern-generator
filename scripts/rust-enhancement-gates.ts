...
/**
 * The enhancement release gates (D118), rebuilt against **Rust** (G-068 M4).
 *
 * These began as `tests/unit/enhancement-calibration.spec.ts` (G-032 M2, criteria 2, 3e, 3f; D113, D115) and drove
 * `buildPattern`, so they went with the TypeScript pipeline in M3 — leaving the Owner's decision that a mode is
 * released only once it passes them with nothing enforcing it. The measurements are unchanged; only the thing being
 * measured moved, from the TypeScript pipeline to the binary production actually runs.
 *
 * Releasing a mode is the Owner's decision (D118), so every mode must pass the safety gates (do no harm, noise,
 * thread palette), while recovery is measured and reported without failing the suite. Set
 * `ENHANCEMENT_CALIBRATION_REPORT=<path>` to write the measurements as JSON.
 *
 * Recovery and do-no-harm compare region boundaries (do two neighbouring cells share a colour?), not exact colours.
 * The first run gated on colour agreement within ΔE 0.06, and even the undegraded photos scored 0.08–0.51 against
 * their own Off pattern: it measured that enhancement changes tone, which is its purpose, rather than recovery or
 * harm. Colour agreement is still reported, ungated (D115).
 *
 * Run with `npm run test:enhancement:rust`, after `cargo build --release`.
 */
...
const bench = openCsBench("enhancement-gates");
...
const NOISE_OPTIONS = { longerSideStitches: 120, colorCount: 8 } as const;
const EDGE_MODES = ["standard", "crisp"] as const;
const PALETTE_MODES = ["dmc", "cosmo", "anchor"] as const;
...
/**
 * Orphan cells as a share of the chart: the same measure the deleted `computePatternDiagnostics` reported, which
 * only ever called `confettiRatio(labelRegions(cellPalette, width, height))` and took the source image for the
 * diagnostics this gate never used.
 */
function confetti(pattern: StitchPattern): number {
  return confettiRatio(labelRegions(pattern.cellPalette, pattern.width, pattern.height));
}

/**
 * Which enhancement stages act on this image, reported beside each measurement so a failure says *why*.
 *
 * This reads the TypeScript `enhance.ts`, which after M3 is the enhancement **preview** rather than the code that
 * built the pattern above (D221). That the two still agree is gated separately, by the parity suite; here it is an
 * ungated diagnostic.
 */
function previewOperationsRun(source: PixelBuffer, mode: Mode) {
...
/**
 * Everything that does not depend on the mode, generated once rather than once per mode.
 *
 * The original spec rebuilt all of this inside the per-mode loop, which cost four identical runs of each Off
 * pattern. With generation in a subprocess that is the bulk of the suite's time.
 */
const BASELINE = {
  fixtures: FIXTURES.map(({ name, source }) => {
    ...
    return {
      ...
      source,
      degraded,
      reference: bench.generate(source, PATTERN_OPTIONS),
      offDegraded: bench.generate(degraded, PATTERN_OPTIONS),
    };
  }),
  noise: EDGE_MODES.map((edgeMode) => ({
    edgeMode,
    off: confetti(bench.generate(NOISY, { ...NOISE_OPTIONS, edgeMode })),
  })),
  threads: PALETTE_MODES.map((paletteMode) => ({
    paletteMode,
    off: bench.generate(degrade(FIXTURES[0].source), { ...PATTERN_OPTIONS, paletteMode }),
  })),
};
...
describe("enhancement release gates", () => {
  it("gates every mode the app offers", () => {
    expect([...releasedEnhancementModes()].filter((m) => m !== "off").sort()).toEqual([...MODES].sort());
  });
  ...
  it.each(MODES)(
    ...
    (mode) => {
      ...
      for (const { name, source, degraded, reference, offDegraded } of BASELINE.fixtures) {
        const modeDegradedPattern = bench.generate(degraded, { ...PATTERN_OPTIONS, enhancementMode: mode });
        const modeOriginalPattern = bench.generate(source, { ...PATTERN_OPTIONS, enhancementMode: mode });
        const offOnDegraded = boundaryAgreement(offDegraded, reference);
      ...
      for (const { edgeMode, off } of BASELINE.noise) {
        const on = confetti(bench.generate(NOISY, { ...NOISE_OPTIONS, edgeMode, enhancementMode: mode }));
        metrics[`noise.${edgeMode}`] = { off, on, previewOperations: previewOperationsRun(NOISY, mode) };
      ...
      for (const { paletteMode, off } of BASELINE.threads) {
        const on = bench.generate(photo, { ...PATTERN_OPTIONS, paletteMode, enhancementMode: mode });
