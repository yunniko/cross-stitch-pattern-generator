...
it.skipIf(!out)("Crisp+ measurements", () => {
  ...
  for (const blurCells of [0, 0.1, 0.25, 0.5, 1]) {
    ...
    for (const colorCount of [8, 16]) {
      for (const edgeMode of ["crisp", "crisp-plus"] as const) {
        report.blur.push({ blurCells, colorCount, edgeMode, ...countBlends(generate(scene.image, edgeMode, colorCount), scene, sets), confidentBoundary: edgeMode === "crisp" ? confidentCrisp : confidentPlus });
      }
    }
  }
  ...
  for (const kind of ["ramp", "radial", "sky"] as const) {
    ...
    for (const colorCount of [8, 16, 32]) {
      ...
      report.gradients.push({ kind, colorCount, crispColours: crisp.palette.length, plusColours: plus.palette.length, differingCells: differing, cells: crisp.cellPalette.length });
    }
  }
