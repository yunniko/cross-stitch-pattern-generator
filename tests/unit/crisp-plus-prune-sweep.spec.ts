...
it.skipIf(!out)(
  ...
  () => {
    ...
    for (const variant of variants) {
      ...
      const options: BlendPruneOptions = variant.off
        ? { ...DEFAULT_BLEND_PRUNE_OPTIONS, maxCandidateInteriorShare: -1 }
        : { ...DEFAULT_BLEND_PRUNE_OPTIONS, ...variant };
      const plus = (image: PixelBuffer, colorCount: number) =>
        buildPattern(image, { longerSideStitches: GRID_WIDTH, colorCount, edgeMode: "crisp-plus", blendPruneOptions: options });
      ...
      for (const { kind, image } of gradients) {
        for (const colorCount of [8, 16, 32]) {
          relabelled += compareAssignments(
            crisp(kind, image, colorCount),
            plus(image, colorCount),
            kind === "ramp" ? RAMP_SEAM_ROWS : []
          ).relabelled;
        }
      }
