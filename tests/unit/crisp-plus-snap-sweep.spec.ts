...
it.skipIf(!out)(
  ...
  () => {
    ...
    for (const maxPerpendicular of [0.15, 0.25, 0.35])
      for (const maxSpanCells of [5, 7]) for (const minSideRun of [2, 3]) variants.push({ maxPerpendicular, maxSpanCells, minSideRun });
    ...
    for (const variant of variants) {
      ...
      const plus = (image: PixelBuffer, colorCount: number) =>
        buildPattern(image, { longerSideStitches: GRID_WIDTH, colorCount, edgeMode: "crisp-plus", transitionSnapOptions: options });
