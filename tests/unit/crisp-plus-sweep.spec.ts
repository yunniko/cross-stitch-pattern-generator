...
it.skipIf(!out)("Crisp+ margin and threshold sweep", () => {
  const scenes = { soft05: regionScene(0.5), soft1: regionScene(1), soft025: regionScene(0.25), line1: thinLineScene(1), line2: thinLineScene(2) };
  ...
  for (const neighborhoodMargin of [0.75, 1, 1.25, 1.5]) {
    for (const confidenceThreshold of [0.7, 0.6, 0.5]) {
      ...
      const generate = (image: Parameters<typeof buildPattern>[0], colorCount: number, edgeMode: "crisp" | "crisp-plus" = "crisp-plus") =>
        buildPattern(image, { longerSideStitches: GRID_WIDTH, colorCount, edgeMode, crispEvidenceLayerOptions: edgeMode === "crisp-plus" ? layer : undefined });
