...
it.skipIf(!out)("Crisp+ versus Crisp generation time", () => {
  const configs = [
  ...
  for (const { label, source, longerSideStitches, colorCount } of configs) {
    ...
    report.push({ label, runs: RUNS, crispMedianMs: Math.round(crisp), crispPlusMedianMs: Math.round(plus), ratio: +(plus / crisp).toFixed(3), crispMs: times.crisp.map(Math.round), crispPlusMs: times["crisp-plus"].map(Math.round) });
