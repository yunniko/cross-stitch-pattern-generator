...
const SOURCES: Array<{ name: string; source: PixelBuffer; grid: [number, number] }> = [
  ...
  { name: "soft ramp just under and over the threshold", source: makeBuffer(120, 40, (x) => [100 + Math.floor(x / 3), 100, 100 + Math.floor(x / 5)]), grid: [40, 13] },
      ...
      for (const [optionsName, options] of OPTION_SETS) {
        ...
        for (let cy = 0; cy < gridHeight; cy++) {
          for (let cx = 0; cx < gridWidth; cx++) {
            ...
            expect(extractBoundaryEvidence(source, gridWidth, gridHeight, cx, cy, options, rows), `${optionsName}, cell ${cx},${cy}`).toStrictEqual(expected);
          }
        }
      }
    });
  ...
  it("keeps the same confident cells in both evidence layers on a real-sized photo", () => {
    ...
    for (const layerOptions of [DEFAULT_CRISP_EVIDENCE_LAYER_OPTIONS, CRISP_PLUS_EVIDENCE_LAYER_OPTIONS]) {
      ...
      for (let cell = 0; cell < 400 * 267; cell++) {
        const cx = cell % 400;
        const evidence = extractBoundaryEvidencePreG047(big, 400, 267, cx, (cell - cx) / 400, layerOptions.boundaryEvidenceOptions, referenceRows);
